...
class RT_PolygonMesh
{
	RT_PolygonMesh::RT_PolygonMesh() { ; }
	RT_PolygonMesh::~RT_PolygonMesh() { ; }

	void RT_PolygonMesh::GetHitProperties(Vertex& v0, Vertex& v1, Vertex& v2, float& u, float& v, int& textureHeight, int& textureWidth, glm::vec3& N, glm::vec2 &texture_coords)
	{
	...
	void RT_PolygonMesh::GetHitProperties(Vertex& v0, Vertex& v1, Vertex& v2, float u, float v, glm::vec3& N)
	{
#ifndef SMOOTH_SHADING
		N = getTriangleNormal(v0.position, v1.position, v2.position);
#else
	...
	void RT_Mesh::RT_PolygonMesh(RT_PolygonMesh::Vertex* vertices, const unsigned int *indices, unsigned int vertices_len, unsigned int indices_len, bool singleSided/*, glm::f32vec3 _color*/, float albedo, RT_Mesh::MATERIAL_TYPE material) {
	...
	RT_Mesh::RT_PolygonMesh(RT_Mesh::TYPE type, std::vector<RT_PolygonMesh::Vertex> vertices, std::vector<unsigned int> indices, bool singleSided, Material material, float albedo, RT_Mesh::MATERIAL_TYPE material_type, std::vector<Texture> textures)
		: _albedo(albedo),
		_material(material),
